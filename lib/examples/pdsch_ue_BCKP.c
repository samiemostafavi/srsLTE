...
#define WRITE_FILE_METRICS

#ifdef WRITE_FILE_METRICS

int METRICS_NUM = 100000000;
...
{
	metrics_handler* myhand = malloc(sizeof(metrics_handler));
	myhand->size = 0;
	myhand->cfo = malloc(sizeof(double)*METRICS_NUM);
	myhand->rsrp = malloc(sizeof(double)*METRICS_NUM);
	myhand->snr = malloc(sizeof(double)*METRICS_NUM);
	myhand->rb_proc = malloc(sizeof(double)*METRICS_NUM);
	myhand->pdcch_miss = malloc(sizeof(double)*METRICS_NUM);
	myhand->pdsch_bler = malloc(sizeof(double)*METRICS_NUM);
	return myhand;
}

void add_metrics(metrics_handler* myhand,double cfo,double rsrp, double snr, double rb_proc, double pdcch_miss, double pdsch_bler)
{
	if(myhand->size == METRICS_NUM)
		return;
	...
	myhand->rsrp[i] = rsrp;
	myhand->snr[i] = snr;
	myhand->rb_proc[i] = rb_proc;
	myhand->pdcch_miss[i] = pdcch_miss;
	myhand->pdsch_bler[i] = pdsch_bler;

	myhand->size++;
}

void finish_metrics(metrics_handler* myhand)
{
	//print stuff to txt file
    	FILE *fpOut;
        if((fpOut = fopen("/tmp/pdsch_ue_metrics.txt", "w")) == NULL)
        {
	  ...
	  free(myhand->cfo);
          ...
          free(myhand->snr);
          free(myhand->rb_proc);
          free(myhand->pdcch_miss);
          free(myhand->pdsch_bler);
          free(myhand);
	  return;
        };
        fprintf(fpOut, "cfo rsrp snr rb_proc pdcch_miss pdsch_bler\n");

        int i;
        for(i=0; i<myhand->size; i++)
        {
            fprintf(fpOut, "%+7.2f", myhand->cfo[i]);
            fprintf(fpOut, " %+5.1f", myhand->rsrp[i]);
            fprintf(fpOut, " %+5.1f", myhand->snr[i]);
            fprintf(fpOut, " %6.2f", myhand->rb_proc[i]);
            fprintf(fpOut, " %5.2f", myhand->pdcch_miss[i]);
            fprintf(fpOut, " %5.2f", myhand->pdsch_bler[i]);
            fprintf(fpOut, "\n");
        }
        fclose(fpOut);
	free(myhand->cfo);
